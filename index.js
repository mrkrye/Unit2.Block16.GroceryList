...
function capitalizeNames() {
  ...
  return items.map((name) => name.name[0].toUpperCase() + name.name.slice(1));
}
...
function getItemPriceByName(name) {
  ...
  return items.filter((item) => item.name === name)[0].price;
}
...
function logCartItems() {
  ...
  cart.forEach((item) => {
    console.log(findItemById(Number(item)).name);
  });
}
...
function calculateTotalCartPrice() {
  ...
  return cart.reduce((sum, item) => {
    return (sum += findItemById(Number(item)).price);
  }, 0);
}
